...
public abstract class FireStoreRepository<E extends Entity> implements CrudRepository<E, String> {
    ...
    public Task<List<E>> findAll() {
        ...
        return this.collectionReference.get().continueWithTask(task -> {
            List<E> list = task.getResult().toObjects(entityClass);
            List<DocumentSnapshot> documents = task.getResult().getDocuments();
            for (int i = 0; i < list.size(); i++) {
                list.get(i).setDocumentId(documents.get(i).getId());
            }
